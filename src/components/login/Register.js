...
function Register() {
        ...
        if(password !== confPassword){
            ...
            .catch(err=>{
                if(err.response.data){
                    if(err.response.data.pseudo) nomErr.innerHTML= err.response.data.pseudo;
                    else nomErr.innerHTML= '';
                    if(err.response.data.email) emailErr.innerHTML= err.response.data.email;
                    else emailErr.innerHTML= '';
                    if(err.response.data.password) passwordErr.innerHTML= err.response.data.password;
                    else passwordErr.innerHTML= '';
                    if(err.response.data.general) generalErr.innerHTML= err.response.data.general;
                }            
                console.log(err);
            })
        }
    ...
    return (
        <div className="login_form">
            ...
            <div className="input_login">
                <input onClick={signUp} type="submit" value="Créer compte" />
            </div>
        </div>
    )
}
